 ...
 */
static av_always_inline int normal_limit(uint8_t *p, ptrdiff_t stride, int E, int II)
{
    LOAD_PIXELS
    return simple_limit(p, stride, E)
        && FFABS(p3-p2) <= II && FFABS(p2-p1) <= II && FFABS(p1-p0) <= II
        && FFABS(q3-q2) <= II && FFABS(q2-q1) <= II && FFABS(q1-q0) <= II;
}
