...
typedef struct {
    DECLARE_ALIGNED(32, QDM2Complex, pComplex)[MPA_MAX_CHANNELS][256];
} QDM2FFT;
...
static void qdm2_fft_generate_tone(QDM2Context *q, FFTTone *tone)
{
    ...
    if (tone->duration >= 3 || tone->cutoff >= 3) {
        tone->pComplex[0].im += c.im;
        tone->pComplex[0].re += c.re;
        tone->pComplex[1].im -= c.im;
        tone->pComplex[1].re -= c.re;
    } else {
        f[1] = -tone->table[4];
        ...
        f[5] = tone->table[2];
        for (i = 0; i < 2; i++) {
            tone->pComplex[fft_cutoff_index_table[tone->cutoff][i]].re +=
                c.re * f[i];
            tone->pComplex[fft_cutoff_index_table[tone->cutoff][i]].im +=
                c.im * ((tone->cutoff <= i) ? -f[i] : f[i]);
        }
        for (i = 0; i < 4; i++) {
            tone->pComplex[i].re += c.re * f[i + 2];
            tone->pComplex[i].im += c.im * f[i + 2];
        }
    }
...
static void qdm2_fft_tone_synthesizer(QDM2Context *q, int sub_packet)
{
    ...
    for (ch = 0; ch < q->channels; ch++) {
        memset(q->fft.pComplex[ch], 0, q->fft_size * sizeof(QDM2Complex));
    }
    ...
    if (q->fft_coefs_min_index[4] >= 0)
        for (i = q->fft_coefs_min_index[4]; i < q->fft_coefs_max_index[4]; i++) {
            ...
            q->fft.pComplex[ch][q->fft_coefs[i].offset + 0].re += c.re;
            q->fft.pComplex[ch][q->fft_coefs[i].offset + 0].im += c.im;
            q->fft.pComplex[ch][q->fft_coefs[i].offset + 1].re -= c.re;
            q->fft.pComplex[ch][q->fft_coefs[i].offset + 1].im -= c.im;
        }
        ...
        if (q->fft_coefs_min_index[i] >= 0) {
            for (j = q->fft_coefs_min_index[i]; j < q->fft_coefs_max_index[i]; j++) {
                ...
                if (offset < q->frequency_range) {
                    ...
                    tone.pComplex = &q->fft.pComplex[ch][offset];
...
static void qdm2_calculate_fft(QDM2Context *q, int channel, int sub_packet)
{
    ...
    int i;
    q->fft.pComplex[channel][0].re *= 2.0f;
    q->fft.pComplex[channel][0].im  = 0.0f;
    q->rdft_ctx.rdft_calc(&q->rdft_ctx, (FFTSample *)q->fft.pComplex[channel]);
    /* add samples to output buffer */
    for (i = 0; i < FFALIGN(q->fft_size, 8); i++) {
        out[0]           += q->fft.pComplex[channel][i].re * gain;
        out[q->channels] += q->fft.pComplex[channel][i].im * gain;
        out              += 2 * q->channels;
    }
}
