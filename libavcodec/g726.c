...
static int16_t g726_decode(G726Context* c, int II)
{
    int dq, re_signal, pk0, fa1, i, tr, ylint, ylfrac, thr2, al, dq0;
    Float11 f;
    int I_sig= II >> (c->code_size - 1);

    dq = inverse_quant(c, II);
    ...
    /* Update Ap */
    c->dms += (c->tbls.F[II]<<4) + ((- c->dms) >> 5);
    c->dml += (c->tbls.F[II]<<4) + ((- c->dml) >> 7);
    if (tr)
        c->ap = 256;
    else {
        c->ap += (-c->ap) >> 4;
    ...
    /* Update Yu and Yl */
    c->yu = av_clip(c->y + c->tbls.W[II] + ((-c->y)>>5), 544, 5120);
    c->yl += c->yu + ((-c->yl)>>6);
